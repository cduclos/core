...
static void SendClassData(AgentConnection *conn)
{
    ...
    {
        if (SendTransaction(conn->connection.physical.sd, rp->item, 0, CF_DONE) == -1)
        {
    ...
    if (SendTransaction(conn->connection.physical.sd, sendbuffer, 0, CF_DONE) == -1)
    {
        Log(LOG_LEVEL_ERR, "Transaction failed. (send: %s)", GetErrorStr());
        return;
    }
...
static void HailExec(AgentConnection *conn, char *peer, char *recvbuffer, char *sendbuffer)
{
    ...
    while (true)
    {
        ...
        if ((n_read = ReceiveTransaction(conn->connection.physical.sd, recvbuffer, NULL)) == -1)
        {
            return;
        }
